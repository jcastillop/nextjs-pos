import { getDatetimeFormatFromString, getDatetimeFormatFromStringLocal, getTodayDatetime } from '@/helpers';
...
export const PrintCierreTurno = forwardRef(({ horaIngreso, totalGalones, totales, gastos, depositos }: IPrintCierreProps, ref) => {
                ...
                <div>
                    <>
                        ...
                        <Grid container spacing={1} sx={{ mt: 0 }} style={{ border: "1px solid grey" }}>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>NOTAS DE DESPACHO</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>GAL.</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>Importe</Typography></Grid>
                            {
                                totalGalones.map((galones: ICierreTurnoPrint) => (
                                    <Grid container key={galones.producto} style={{ marginLeft: 10 }}>
                                        <Grid item xs={4} style={{ paddingTop: 0 }}>{galones.producto}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>{(galones.despacho_galones ? galones.despacho_galones : 0).toFixed(3)}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>S/{(galones.despacho_soles ? galones.despacho_soles : 0).toFixed(2)}</Grid>
                                    </Grid>
                                ))
                            }

                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>SERAFIN</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>GAL.</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>Importe</Typography></Grid>
                            {
                                totalGalones.map((galones: ICierreTurnoPrint) => (
                                    <Grid container key={galones.producto} style={{ marginLeft: 10 }}>
                                        <Grid item xs={4} style={{ paddingTop: 0 }}>{galones.producto}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>{(galones.calibracion_galones ? galones.calibracion_galones : 0).toFixed(3)}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>S/{(galones.calibracion_soles ? galones.calibracion_soles : 0).toFixed(2)}</Grid>
                                    </Grid>
                                ))
                            }
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>TOTAL</Grid>
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>{tot_notas_serafin_galones.toFixed(3)}</Grid>
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>S/ {tot_notas_serafin_soles.toFixed(2)}</Grid>
                        </Grid>
                        ...
                        <Grid container spacing={1} sx={{ mt: 0 }} style={{ border: "1px solid grey" }}>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>PRODUCTO</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>GAL.</Typography></Grid>
                            <Grid item xs={4}><Typography sx={{ fontWeight: 'bold' }}>Importe</Typography></Grid>
                            {
                                totalGalones.map((galones: ICierreTurnoPrint) => (
                                    <Grid container key={galones.producto} style={{ marginLeft: 10 }}>
                                        <Grid item xs={4} style={{ paddingTop: 0 }}>{galones.producto}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>{(galones.total_galones ? galones.total_galones : 0).toFixed(3)}</Grid>
                                        <Grid item xs={4} style={{ paddingTop: 0, paddingLeft: 5 }}>S/ {(galones.total_soles ? galones.total_soles : 0).toFixed(2)}</Grid>
                                    </Grid>
                                ))
                            }
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>TOTAL</Grid>
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>{tot_venta_galones.toFixed(3)}</Grid>
                            <Grid item xs={4} style={{ paddingTop: 0, fontWeight: 'bold', marginTop: 2, fontSize: 15 }}>S/ {tot_venta_soles.toFixed(2)}</Grid>
                        </Grid>
                        ...
                        <Grid container spacing={1} sx={{ mt: 0 }} style={{ border: "1px solid grey" }}>
                            <Grid container style={{ marginLeft: 10 }}>
                                ...
                                <Grid item xs={3}><Typography sx={{ fontWeight: 'bold' }}>Importe</Typography></Grid>
                        ...
                        }
                        {
                            depositos && depositos.length > 0 && (
                                <>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: 15, marginTop: 8 }}><b>DEPOSITOS</b></div>
                                    </div>
                                    <Grid container spacing={1} sx={{ mt: 0 }} style={{ border: "1px solid grey" }}>
                                        <Grid item xs={6} justifyContent='end'><Typography sx={{ fontWeight: 'bold' }}>FECHA</Typography></Grid>
                                        <Grid item xs={6} justifyContent='end'><Typography sx={{ fontWeight: 'bold' }}>MONTO</Typography></Grid>
                                        {
                                            depositos.map((deposito: IDeposito) => (
                                                <Grid container key={deposito.id} style={{ marginLeft: 10 }}>
                                                    <Grid item xs={6} style={{ paddingTop: 0 }}>{getDatetimeFormatFromStringLocal(deposito.fecha)}</Grid>
                                                    <Grid item xs={6} style={{ paddingTop: 0 }}>S/ {deposito.monto.toFixed(2)}</Grid>
                                                </Grid>
                                            ))
                                        }
                                    </Grid>
